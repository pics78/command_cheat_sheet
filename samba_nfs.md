...
- NFSサーバ
    ...
    - 必要なプロセス
        - RPCプログラム番号とポート番号のマッピングデーモン  
        以前はportmap（portmapper）
        今はrpcbind（IPv6対応, NFSv3までは必須デーモン）
        - nfsd  
        ファイルシステムのエクスポートやNFSクライアントからの要求などを処理する中心的なデーモン  
        全てのバージョンのサーバ側に必要
        - rpc.mountd（mountd）  
        NFSクライアントからのマウントやアンマウントの要求を受け付けるデーモン  
        全てのバージョンのサーバ側に必要
        - rpc.idmapd  
        /etc/idmapd.confの設定により、クライアントのユーザ名、グループ名とUID、GIDのマッピングを行う  
        NFSv4のみ、サーバとクライアントに同じ設定が必要
